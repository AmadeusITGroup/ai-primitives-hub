...
>
> **MCP extraction (GitHub):** `processRelease` reads MCP data from the deployment manifest's top-level `mcpServers` and `mcpInputs` fields (written by `generate-manifest.js`). When `mcpInputs` is present it is attached to the bundle as-is, enabling VS Code to prompt the user for secrets and configurable values at install time. This mirrors the behaviour of `AwesomeCopilotAdapter`, which reads the same keys from `.collection.yml` files directly.
>
