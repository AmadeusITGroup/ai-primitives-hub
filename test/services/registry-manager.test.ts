...
suite('RegistryManager - Repository Workflow Parity', () => {
  let sandbox: sinon.SinonSandbox;
  let mockContext: vscode.ExtensionContext;
  let manager: RegistryManager;
  let mockStorage: sinon.SinonStubbedInstance<RegistryStorage>;
  let repositoryBundlesChangedCount: number;
  ...
  const bundle: Bundle = {
    ...
    tags: [],
    sourceId: source.id,
    downloadUrl: 'file:///repo-bundle.zip',
    manifestUrl: 'file:///deployment-manifest.yml',
    lastUpdated: new Date().toISOString(),
    environments: [],
    size: '1KB',
    dependencies: [],
  ...
  const installedBundle: InstalledBundle = {
    ...
    installedAt: new Date().toISOString(),
    scope: 'repository',
    installPath: '/mock/workspace/.github',
    manifest: { id: bundle.id, name: bundle.name, version: bundle.version } as any
  };
  ...
  setup(() => {
    ...
    repositoryBundlesChangedCount = 0;
    manager.onRepositoryBundlesChanged(() => {
  ...
  test('installBundle should refresh repository listeners for repository scope', async () => {
    sandbox.stub(manager as any, 'resolveInstallationBundle').resolves(bundle);
    sandbox.stub(manager as any, 'getSourceForBundle').resolves(source);
    sandbox.stub(manager as any, 'downloadAndInstall').resolves(installedBundle);

    await manager.installBundle(bundle.id, { scope: 'repository' });
  ...
  test('uninstallBundle should refresh repository listeners for repository scope', async () => {
    ...
    sandbox.stub(scopeSelectionUI, 'getWorkspaceRoot').returns('/mock/workspace');
    sandbox.stub(LockfileManager, 'getInstance').returns({
      getInstalledBundles: sandbox.stub().resolves([installedBundle])
    } as any);
    sandbox.stub((manager as any).installer, 'uninstall').resolves();

    await manager.uninstallBundle(bundle.id, 'repository');
  ...
  test('updateBundle should refresh repository listeners for repository scope', async () => {
    mockStorage.getSources.resolves([source]);
    sandbox.stub(manager, 'listInstalledBundles').resolves([installedBundle]);
    sandbox.stub(manager as any, 'checkLocalModificationsBeforeUpdate').resolves();
    sandbox.stub(manager as any, 'getBundleDetails').resolves({ ...bundle, version: '2.0.0' });
    sandbox.stub(RepositoryAdapterFactory, 'create').returns({
      downloadBundle: sandbox.stub().resolves(Buffer.from('updated'))
    } as any);
    sandbox.stub((manager as any).installer, 'update').resolves({
      ...installedBundle,
      version: '2.0.0'
    });

    await manager.updateBundle(bundle.id);
