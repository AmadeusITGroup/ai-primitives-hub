 ...
 * into `gh` CLI.
 *
 * Distinct from `core`'s `TokenProvider` port (host-aware, one token per
 * request, used by `GitHubApiClient`): this module is the harvester's own
 * resolution strategy — explicit -> `GITHUB_TOKEN` -> `GH_TOKEN` -> `gh`
 * CLI — used once per harvest run to produce a single token, which the
 * caller then wraps in a `core` `TokenProvider` (e.g. a constant-returning
 * one) to hand to `GitHubApiClient`. Ported from the reference branch's
 * `infra/src/harvest/token-provider.ts` unchanged (no core/infra port
